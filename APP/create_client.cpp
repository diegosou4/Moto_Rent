...
void create_client::on_pushButton_2_clicked()
{
    ...
    QSqlQuery query;
    bool lines;
    lines = checkQuery.islineEmpty(ui->line_name->text(),ui->line_numero->text(),ui->line_nif->text(),ui->line_adress->text(),ui->line_id_moto->text());   // Vai checkar se tem alguma linha vazia


    if(lines){
        QMessageBox::warning(this,"Erro","Algum campo esta vazio");
    } else {
    checkQuery.getvaluesline(ui->line_name->text(),ui->line_numero->text(),ui->line_nif->text(),ui->line_adress->text(),ui->date_start->text(),ui->line_id_moto->text());
    int motoisEmpety;
    motoisEmpety= checkQuery.check_moto_rent(query);
    if(motoisEmpety == 1){
